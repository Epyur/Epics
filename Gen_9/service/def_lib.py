...
def excel_column_to_dataframe_headers(title_frame, sheet_name, column_index):
    # Получаем значения из указанного столбца
    new_headers = title_frame.iloc[:, column_index].tolist()
...
def TakeRequest(data_frame,
                    filtre_column=None,
                    searched_index = None,
                title_frame = None):
    try:

        d_f = data_frame.reset_index()
    ...
    try:
        df_new = excel_column_to_dataframe_headers(title_frame, 'Sheet1', 1)
        # Устанавливаем ключи
        target_df = df_new.set_index('index')
        original_columns = target_df.columns
        source_df = filtered_df.set_index('index')
